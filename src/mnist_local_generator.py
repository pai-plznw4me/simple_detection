...
from utils import draw_rectangle, xyxy2xywh, xywh2xyxy, plot_images, images_with_rectangles
...
def mnist_localization_generator(train_shape, test_shape, background=True, n_sample=None, image_size_range=None,
    ...
    Args:
        ...
        image_size_range : tuple, (size_min:int, size_max:int),
            이미지 내 객체의 size 범위 값을 제공합니다. 해당 값을 min 값 부터 max 값까지 하나의 값을 추출합니다.
        image_ratio_range : tuple, (ratio_min:float, ratio_maxfloat),
            이미지 내 객체의 ratio 범위 값을 제공합니다. 해당 값을 min 값 부터 max 값까지 하나의 값을 추출합니다.
    :return:
